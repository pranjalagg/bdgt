...
<Modal id="bucket-form" title={editingBucket ? 'Edit Bucket' : 'Add Bucket'}>
  <form on:submit|preventDefault={handleSubmit} class="space-y-4">
    <div>
      ...
      <input id="name" type="text" bind:value={newBucketName} class="mt-1 w-full rounded-lg border px-3 py-2" required />
    </div>
    <div>
      <label for="color" class="block text-sm font-medium text-gray-700">Color</label>
      <input id="color" type="color" bind:value={newBucketColor} class="mt-1 h-10 w-full rounded-lg border" />
    </div>
